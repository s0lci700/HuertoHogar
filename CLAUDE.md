...
El repositorio está indexado con GitNexus como **HuertoHogar** (166 símbolos,
350 relaciones, 20 flujos de ejecución). El índice vive en `.gitnexus/`, que
está en `.gitignore`: cada quien lo genera en su máquina con
`npx gitnexus analyze`.

Sirve para responder "¿qué llama a esta función?" o "¿qué se rompe si cambio
esto?" sin revisar los ocho módulos a mano:

```bash
node .gitnexus/run.cjs impact "agregarProducto" --direction upstream --repo .
node .gitnexus/run.cjs context "renderizarCarrito" --repo .
```

Es una ayuda, no un procedimiento obligatorio: EV1 son unas 970 líneas de
JavaScript plano y la mayoría de los cambios no necesitan análisis de grafo.
Donde sí conviene consultarlo es antes de renombrar o mover algo que se use en
varias páginas, porque estos scripts son globales y dos funciones con el mismo
nombre se pisan en silencio (ya pasó con `formatearPrecio` y `obtenerUsuario`).

> Este bloque está recortado a mano. `npx gitnexus analyze` lo regenera completo
> y hay que volver a recortarlo.
<!-- gitnexus:end -->
